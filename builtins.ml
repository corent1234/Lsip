...
let rec eval_add arg1 arg2 =
  match arg1, arg2 with
  | Atom a, Atom b ->Atom (string_of_int (int_of_string a + (int_of_string b)))
  | _ -> raise (Error "Can not add lists")

(* interpret (sub arg1 arg2) *)
let rec eval_sub arg1 arg2 =
  match arg1, arg2 with
  | Atom a, Atom b ->Atom (string_of_int( int_of_string a - (int_of_string b)))
  | _ -> raise (Error "Can not substract lists")

(* interpret (add arg1 arg2) *)
let rec eval_mul arg1 arg2 =
  match arg1, arg2 with
  | Atom a, Atom b ->Atom( string_of_int( int_of_string a * (int_of_string b)))
  | _ -> raise (Error "Can not multiply lists")


(* interpret (l arg1 arg2) *)
let rec eval_l arg1 arg2 =
  match arg1, arg2 with
  | Atom a, Atom b ->if int_of_string a < (int_of_string b)
                                          then Atom "t"
                                           else Atom "nil"
  | _ -> raise (Error "Can not compare lists")


(* interpret (g arg1 arg2) *)
let rec eval_g arg1 arg2 =
  match arg1, arg2 with
  | Atom a, Atom b -> if int_of_string a > (int_of_string b)
                                          then Atom "t"
                                           else Atom "nil"
...
let rec eval_add_list arg =
 let rec aux arg =
  match arg with
  | Atom "nil" -> 0
  | Atom _ -> raise (Error "PV=nRT")
  | Cons(Atom car,cdr) ->(int_of_string  car) + (aux cdr)
  | _ -> raise (Error "Can not add lists") in
 Atom (string_of_int (aux arg))

(* interpret (cond args) *)
(* interpret (if cond then_form else_forms) *)
let rec eval_cond args =
  begin
    check_min_nargs args 2 "if";
    let cond_form = nth 0 args in
    let then_form = nth 1 args in
    let rest_forms = drop 2 args in
    let e_cond_form = eval cond_form in
    if not (is_nil e_cond_form)
    then
      eval then_form
    else
      eval_cond rest_forms
  end
